            ...
            return Double.parseDouble(
                    new BufferedReader(
                            new InputStreamReader(
                                    Runtime.getRuntime().exec("ps -p " + ManagementFactory.getRuntimeMXBean().getName().split("@")[0] + "-o %cpu | tail -n 1").getInputStream()
                            )
                    ).readLine()
            );
        } catch (Exception e) {
    ...
    public double getMemoryUsage() {
        try {
            return Double.parseDouble(
                    new BufferedReader(
                            new InputStreamReader(
                                    Runtime.getRuntime().exec("ps -p " + "processid" + "-o %mem | tail -n 1").getInputStream()
                            )
                    ).readLine()
            );
        } catch (Exception e) {
